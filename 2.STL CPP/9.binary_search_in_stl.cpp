...
int main() { 
        ...
        //A means it points to the first iterator that is on the first element 
        //A+n means the iterator point on the ending postion, that is points to the position after the last element

        res = binary_search(A, A + n, 4);
        //return true
        // This means 4 is present in the array

        //here rather than writing such big code ,just use stl and got the answer
        ...
        // answer is 4 becuase 7 is not there so we find next greatest element which is 9
        ...
        // Answer is 6 because 10 is greater than the last element 9
        //so this return index is 6 because last is 9 which at 5 index but 10 is greater than 9 so it will be go to next iterator which is 6th
        ...
        ind = lower_bound(vec.begin(), vec.end(), 4) - vec.begin();
        // The same logic applies when using iterators with other container types like vector
        ...
        // For 10, it will point to the position just after the last element since 10 is greater than the last element 9
        //it will iterator after the last element which 9
        // the index should be 6

         // Syntax
         ind = upper_bound(vec.begin(), vec.end(), 4) - vec.begin();
        ...
        */
         ...
         n = sizeof(Q) / sizeof(Q[0]); // Update n for array Q
        ...
        */
        ...
        // Examples:
        //x = 4   //answer is 4 because,first its find index 5th which is 9 there then ind-- will send back to 4
